...
export function encodeDocsUri(query?: Query): Uri {
  return Uri.parse(`vuesax-helper://search?${JSON.stringify(query)}`);
}
...
export class App {
  ...
  openDocs(query?: Query, title = 'vuesax-helper', editor = window.activeTextEditor){
  ...
  getWebviewContent(query: Query) {
    const config = workspace.getConfiguration('vuesax-helper');
...
export class VsDocsContentProvider implements TextDocumentContentProvider {
...
export class VsCompletionItemProvider implements CompletionItemProvider {
  ...
  buildTagSuggestion(tag, tagVal, id) {
    ...
    let index = 0;
    let that = this;
    function build(tag, {subtags, defaults, innerText, min}, snippets) {
      let attrs = '';
      defaults && defaults.forEach((item, i) => {
        attrs += ` ${item}=${that.quotes}$${'{' + (index + i + 1)}${that.getAttrValues(tag, item).length > 0 ? `|${that.getAttrValues(tag, item)}|` : '' }}${that.quotes}`;
      });
      snippets.push(`${index > 0 ? '<':''}${tag}${attrs} ${ min ? '/' : '' }>`);
      index++;
      subtags && subtags.forEach(item => build(item, TAGS[item], snippets));
      if (innerText) {;
        snippets.push('${' + `${index + 1}:${innerText}}`);
      }
      // snippets.push(that.getAttrValues(tag, item));
      if(!min) {
        snippets.push(`</${tag}>`);
      }
    };
